...
- Open the 📖 button for a conventions cheat sheet: a level-by-level index of
  the H-group conventions, each linking to its section on hanabi.github.io.
...
npm run scrape:conventions     # refresh cheat-sheet data from hanabi.github.io
```

The conventions cheat sheet (the 📖 button) is backed by
`src/lib/data/conventions.ts`, generated by `scripts/scrape-conventions.mjs`.
Re-run `npm run scrape:conventions` when the upstream H-group conventions
change.
